...
internal typealias OAut2hCompletionHandler = (TokenResult?, OAuth2Error?) -> Void
...
internal class OAuth2Client {
     ...
     */
    internal func requestOAuthTokenWithCredentials(_ credentials: Credentials, handler : @escaping OAut2hCompletionHandler) {
    ...
    * @param completionHandler closures
    */
    internal func requestOAuthTokenWithUsername(_ ropc: ROPCCredentials, handler : @escaping OAut2hCompletionHandler) {
    ...
    fileprivate func _createAuthorizationHeader(clientID: String, clientSecret: String) -> (String) {
        ...
        if let base64String = utf8str?.base64EncodedString(options: NSData.Base64EncodingOptions(rawValue: 0)) {
        ...
        if let d = data {
            do {
                ...
                if let jsonDic = jsonObject as? [String: Any] {
                    ...
                    if let accessToken = jsonDic["access_token"] as? String {
                        ...
                        return TokenResult(accessToken: accessToken, refreshToken: refreshToken, expiresIn: Double(expiresIn), timestamp: currentTime)
                    }
                }
